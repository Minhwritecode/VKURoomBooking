...
rows = [
    ...
    (17, "Admin account management", "Tìm user, cấp/hạ User/Admin qua Callable Functions + Admin SDK; chặn tự nâng quyền và hạ Admin cuối cùng."),
    (18, "Motion/UI system", "Morph Three.js native/web fallback, responsive, safe-area, reduce motion."),
    (19, "Docker & Firebase", "Node API + Compose; Auth, Firestore users/rooms/bookings/waitlist/reviews, transaction, Storage and Security Rules."),
    (20, "MacBook PWA", "Manifest + service worker shell cache, standalone web output."),
]
...
story = [
    ...
    Table([[P("Student", "SmallCustom"), P("Dinh Tran Tien Minh · 23IT162", "SmallCustom")], [P("Date", "SmallCustom"), P("18/09/2026", "SmallCustom")], [P("Repository", "SmallCustom"), P("github.com/Minhwritecode/VKURoomBooking", "SmallCustom")], [P("Live demo", "SmallCustom"), P("vku-room-booking-michael.vercel.app", "SmallCustom")]], colWidths=[33 * mm, 130 * mm], style=TableStyle([("BACKGROUND", (0, 0), (0, -1), PALE), ("GRID", (0, 0), (-1, -1), 0.35, colors.HexColor("#D7E5F0")), ("VALIGN", (0, 0), (-1, -1), "TOP"), ("LEFTPADDING", (0, 0), (-1, -1), 7), ("TOPPADDING", (0, 0), (-1, -1), 6), ("BOTTOMPADDING", (0, 0), (-1, -1), 6)])),
    Spacer(1, 10 * mm),
    ...
    P("Expo / React Native / Expo Web → Native Stack + Bottom Tabs → responsive UI → Zustand + AsyncStorage cache → Firebase Authentication + Firestore + Storage/Security Rules → Callable Functions + Admin SDK → Node REST API in Docker → PWA manifest + service-worker cache.", "CodeCustom"),
    ...
    P("Conflict prevention: UI khóa slot đã biết; Firebase runTransaction dùng key roomId_dateKey_slotId; backend trả 409 nếu trùng. Offline: AsyncStorage giữ dữ liệu và syncQueue retry tuần tự khi NetInfo báo online; xung đột được gỡ khỏi queue. QR: expo-camera hỗ trợ native, manual code fallback cho desktop/permission failure. Motion: native expo-gl, web canvas và fallback tĩnh giúp trải nghiệm mượt hơn. Admin: Firestore/Storage ghi thật khi tài khoản có role admin. Role management: Callable Function + Admin SDK cập nhật claims/profile, chặn self-escalation và hạ Admin cuối cùng.", "BodyCustom"),
